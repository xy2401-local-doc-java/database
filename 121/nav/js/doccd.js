...
var linkcategories = {
    rank_advanced      : "http://www.oracle.com/pls/db121/ranked?advanced=1",
    homepage           : "../index.htm",
    master_index       : "../nav/mindx.htm",
    master_glossary    : "../nav/mgloss.htm",
    catalog_views      : "../nav/catalog_views.htm",
    sql_keywords       : "../nav/sql_keywords.htm",
    initora            : "../nav/initora.htm",
    lookup_errmg       : "../nav/lookup.htm?id=ERRMG"
  };
...
var l10n = {
    ...
    acronyms                                  : "Acronyms",
    initialization_parameters                 : "Initialization Parameters",
    ...
    content_management_and_unstructured_data  : "Content Management and Unstructured Data",
...
function addThisDocument() {
    var heading, a, li1, ul, thisDocument,
      ...
      nav_cookie_name = "ORA_" + "E50529_01" + "_NAV", error404 = 0,
  ...
  function putSocialIcons() {

    function socialStyles(attachTwitter, attachYouTube, attachRSS) {
      var cssTexts, startPos = -14, gapPos = 20, socialimagelink;
      if (!attachTwitter && !attachYouTube && !attachRSS) {
        return;
      }
      cssTexts = '\n' + "#SOCIALICONS { position: relative; margin-bottom: 6ex;}" + '\n' +
        "#SOCIALICONS li {margin: 0;padding: 0;list-style: none; position: absolute; top: 1ex;}" + '\n' +
        "#SOCIALICONS a { height: 16px; display: block;}" + '\n';

      socialimagelink = "images/social-icons.png";
      addSocialStyle(cssTexts);
      if (!window.location.href.match(new RegExp("\/nav\/", '').ignoreCase) && rssdiv) {
        socialimagelink = "nav/images/social-icons.png";
      } else if (window.location.href.match(new RegExp("\/nav\/", '').ignoreCase)) {
        socialimagelink = "images/social-icons.png";
      } else {
        socialimagelink = "../nav/images/social-icons.png";
      }
      ...
      if (attachTwitter) {
        startPos += gapPos;
        cssTexts = '\n' + "#TWITTERICON {left: " + startPos + "px;width: 16px;background: url(" + socialimagelink + ") -244px 0;}" + '\n' +
          "#TWITTERICON a:hover {background: url(" + socialimagelink + ") -264px 0;}" + '\n';
        addSocialStyle(cssTexts);
      }
      if (attachYouTube) {
        startPos += gapPos;
        cssTexts = '\n' + "#YOUTUBEICON {left: " + startPos + "px; width: 16px; background: url(" + socialimagelink + ") -124px 0;}" + '\n' +
          "#YOUTUBEICON a:hover {background: url(" + socialimagelink + ") -144px 0;}" + '\n';
        addSocialStyle(cssTexts);
      }

      if (attachRSS) {
        startPos += gapPos;
        cssTexts = '\n' + "#RSSICON {left: " + startPos + "px;width: 16px;background: url(" + socialimagelink + ") -284px 0;}" + '\n' +
          "#RSSICON a:hover {background: url(" + socialimagelink + ") -304px 0;}" + '\n';
        addSocialStyle(cssTexts);
      }
    }

    function addSocialStyle(cssTexts) {
      var styler;
      styler = document.createElement("style");
      styler.type = "text/css";
      if (styler.styleSheet) {
        styler.styleSheet.cssText = cssTexts;
      } else {
        styler.appendChild(document.createTextNode(cssTexts));
      }
      document.getElementsByTagName("head")[0].appendChild(styler);
    }

    function addSocialIcons(attachTwitter, attachYouTube, attachRSS) {
      var rssLink, twitterLink, youtubeLink, socialUL, liSocial;
      if (!attachYouTube && !attachTwitter && !attachRSS) {
        return;
      }
      // style the icons.
      socialStyles(attachTwitter, attachYouTube, attachRSS);
      // build the display using unordered list.
      socialUL = document.createElement("ul");
      socialUL.id = "SOCIALICONS";
      // add only if asked for.
      if (attachTwitter) {
        twitterLink = document.createElement("a");
        twitterLink.href = "https://twitter.com/OracleDBInfoDev";
        twitterLink.title = "Follow us on Twitter";
        liSocial = document.createElement("li");
        liSocial.id = "TWITTERICON";
        liSocial.appendChild(twitterLink);
        socialUL.appendChild(liSocial);
      }
      if (attachYouTube) {
        youtubeLink = document.createElement("a");
        youtubeLink.href = "http://www.youtube.com/user/OracleDBVision";
        youtubeLink.title = "Watch us on YouTube";
        liSocial = document.createElement("li");
        liSocial.id = "YOUTUBEICON";
        liSocial.appendChild(youtubeLink);
        socialUL.appendChild(liSocial);
      }
      if (attachRSS) {
        rssLink = document.createElement("a");
        if (!window.location.href.match(/\/nav\//) && rssdiv) {
          rssLink.href = "nav/rss.xml";
        ...
        liSocial = document.createElement("li");
        liSocial.id = "RSSICON";
        liSocial.appendChild(rssLink);
        socialUL.appendChild(liSocial);
      }
      return socialUL;
    }

    var rssdiv, rssdivparent, socialElement;
    rssdiv = document.getElementById('rss_feeds');
    socialElement = addSocialIcons(true, true, true);
    if (rssdiv) {
      // This would be the case of library index or portlets
      rssdivparent = rssdiv.parentNode;
      if (rssdivparent) {
        rssdiv.parentNode.removeChild(rssdiv);
        rssdivparent.appendChild(socialElement);
      }
      return;
    }
  ...
  if (!tSchema.match(/^as\d+/) && !tSchema.match(/^fa\d+/) &&
    ...
    //tahitiNav.appendChild(putSocialIcons());
  }
  ...
  function addQuickLookup() {
    ...
    */
    ...
    link(l10n.installation, "../nav/portal_11.htm");
    link(l10n.getting_started, "../nav/portal_1.htm");
    link(l10n.administration, "../nav/portal_4.htm");
    link(l10n.application_development, "../nav/portal_5.htm");
    link(l10n.grid_computing, "../nav/portal_16.htm");
    link(l10n.high_availability, "../nav/portal_14.htm");
    link(l10n.data_warehousing, "../nav/portal_6.htm");
    link(l10n.content_management_and_unstructured_data, "../nav/portal_7.htm");
    link(l10n.information_integration, "../nav/portal_15.htm");
    link(l10n.security, "../nav/portal_25.htm");
    link(l10n.videos, "../nav/portal_30.htm");
  ...
  function addSubCategories() {
    ...
    link(l10n.data_dictionary, linkcategories.catalog_views);
    link(l10n.sql_keywords, linkcategories.sql_keywords);

    link(l10n.initialization_parameters, linkcategories.initora);
    link(l10n.error_messages, linkcategories.lookup_errmg);
